...
//Class declaration
class GenJpsiToEleEleFilter : public edm::global::EDFilter<> {
public:
  explicit GenJpsiToEleEleFilter(const edm::ParameterSet&);
...
private:
  ...
  const double elePtCut_, eleEtaCut_, nJpsis_, eledRCut_ ;
};

//Constructor
GenJpsiToEleEleFilter::GenJpsiToEleEleFilter(const edm::ParameterSet& params)
    : token_(consumes<reco::GenParticleCollection>(params.getParameter<edm::InputTag>("src"))),
      ...
      nJpsis_(params.getParameter<double>("nJpsis")),
      eledRCut_(params.getParameter<double>("eledRCut")) {}

bool GenJpsiToEleEleFilter::filter(edm::StreamID, edm::Event& evt, const edm::EventSetup& params) const {
  using namespace std;
  using namespace edm;
  ...
  unsigned JpsiToEleEleCandidate = 0;
  for (reco::GenParticleCollection::const_iterator iGen = genParticles->begin(); iGen != genParticles->end(); ++iGen) {
    if ( abs(iGen->pdgId()) != 443 || iGen->numberOfDaughters() != 2 ) continue;
    ...
    ++JpsiToEleEleCandidate;
  }
  return (JpsiToEleEleCandidate >= nJpsis_);  //Return boolean whether event passes cut values
}

// Define module as a plug-in
DEFINE_FWK_MODULE(GenJpsiToEleEleFilter);
